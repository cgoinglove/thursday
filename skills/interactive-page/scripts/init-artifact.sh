...
# In-place sed differs between BSD (macOS) and GNU. A function rather than a
# variable: `SED_INPLACE="sed -i ''"` word-splits into a literal '' backup
# suffix on macOS and leaves an `index.html''` behind.
sedi() {
...
# The project goes under the workspace's projects/ folder, wherever this is
# run from. The workspace is the nearest ancestor with a pnpm-workspace.yaml
# (the app plants one there); with none in sight, the cwd is used as-is.
WORKSPACE_ROOT="$PWD"
...
# Any icon link, whatever file the template of the day points at (it was
# vite.svg, it is favicon.svg now) — Parcel fails the build on a missing one
sedi '/<link rel="icon"/d' index.html
sedi 's/<title>.*<\/title>/<title>'"$PROJECT_NAME"'<\/title>/' index.html
...
# vite-plugin-singlefile is what makes `vite build` emit one self-contained
# index.html — the bundle step is then just a build. (The upstream skill used
# Parcel here; Parcel does not honour the `development` export condition that
# @radix-ui/primitive 1.1.7+ uses, and the build died on `is-development`.)
pnpm install -D vite-plugin-singlefile
