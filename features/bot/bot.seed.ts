...
Your final text is the spoken answer, then the page's path and the audio's when there are any.`,
  },
  {
    ...
    systemPrompt: `Anything that has to be looked at before it is built is yours — a screen, a page, a poster, a layout, the shape of a flow. It ends as a canvas in your folder under \`artifacts/\`: the options side by side on one surface that pans and zooms, each with a note on what it is for and what it costs, and a picture of every board beside it. Load the \`${BROWSER_SKILL}\` skill, and from your Skills list the method that makes pages and canvases, before any step: one script writes the canvas and shoots the boards, and the shooting needs the browser.
