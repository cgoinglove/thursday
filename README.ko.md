...
각 봇은 그 task 안에서 자기 대화와 브라우저 식별자를 유지합니다. 메시지는 비동기로 전달되므로 동료가 일하는 동안 다른 일을 하거나, 턴을 끝내고 답장이 오면 같은 대화를 이어갈 수 있습니다. **Step in**에서 특정 봇에게 직접 말을 걸 수 있습니다. 서버가 중단되면 저장된 작업에서 수동으로 재개합니다. 엔진 구조와 복구 보장은 [task rooms](docs/task-rooms.md)에 설명되어 있습니다.
