...
![Jarvis가 일을 나누고, Analyst가 숫자를 가져오고, 페이지 한 장이 나온 방 — 아무도 타이핑하지 않았다](docs/images/thread-room.png)

이번 주 시장이 어떻게 움직였는지 한 페이지로 달라고 해보세요. **Jarvis**가 일을 받아 계획을 적어두고 조사를 넘깁니다. **Analyst**가 숫자를 공개된 곳에서 직접 가져와 출처를 남기고 그림으로 그립니다. Jarvis가 돌아온 결과를 열어 계획과 맞는지 확인하고 페이지로 묶어 Thursday에게 넘기면, 나는 한 문장으로 듣습니다.
...
Thursday에는 계획하는 Jarvis, 숫자를 찾아 정리하는 Analyst, 반복되는 일을 스크립트로 만드는 Lambda가 기본으로 들어 있습니다. 이름 하나와 무엇을 위한 봇인지 한 문장이면 내 봇도 만들 수 있고, 실제 결정이 필요할 때는 봇이 나에게 묻습니다.
