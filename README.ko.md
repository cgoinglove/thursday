...
올해 에이전트 하네스는 정말 좋아졌다. Claude Code, OpenClaw, Hermes — 전부 읽고 타이핑한다. 그리고 아주 잘한다.

**다만 아무도 듣지는 않는다.**

생각을 못 해서가 아니다. 음성이 하네스의 단 하나뿐인 일 — 긴 루프를 돈다 — 을 정면으로 깨기 때문이다. 브라우저를 여는 음성 모델은 90초를 조용히 있는다. 조용한 통화는 죽은 통화다. 아무도 수화기를 들고 그걸 견디지 않는다.
