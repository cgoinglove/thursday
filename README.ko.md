...
첫 화면에 OpenAI API 키를 넣고 **“hey thursday”**라고 말하세요. Thursday 계정도 `.env` 파일도 필요 없습니다.
...
## 한 문장이면, 봇 둘이 알아서 나눠 맡습니다.

Thursday는 요청을 에이전트 하나에게 던져두지 않습니다. 봇들이 서로 이야기하며 일을 나눕니다.

![Planner가 일을 나누고, Analyst가 숫자를 가져오고, 페이지 한 장이 나온 방 — 아무도 타이핑하지 않았다](docs/images/thread-room.png)

이번 주 시장이 어떻게 움직였는지 한 페이지로 달라고 해보세요. **Planner**가 일을 받아 계획을 적어두고 조사를 넘깁니다. **Analyst**가 숫자를 공개된 곳에서 직접 가져와 출처를 남기고 그림으로 그립니다. Planner가 돌아온 결과를 열어 계획과 맞는지 확인하고 페이지로 묶어 Thursday에게 넘기면, 나는 한 문장으로 듣습니다.

주고받은 말은 전부 남습니다. 누가 무엇을 했는지 궁금하면 방을 열어 읽고, 더 시킬 게 있으면 그 자리에 적으면 멈춘 곳에서 이어갑니다.

각 봇은 그 스레드 안에서 자기 대화와 브라우저 식별자를 유지합니다. 메시지는 비동기로 전달되므로 동료가 일하는 동안 다른 일을 하거나, 턴을 끝내고 답장이 오면 같은 대화를 이어갈 수 있습니다. **Step in**에서 특정 봇에게 직접 말을 걸 수 있습니다. 서버가 중단되면 저장된 작업에서 수동으로 재개합니다. 엔진 구조와 복구 보장은 [thread rooms](docs/thread-rooms.md)에 설명되어 있습니다.
...
Thursday에는 다른 봇이 맡지 않는 일을 뭐든 하는 Jarvis, 숫자를 찾아 정리하는 Analyst, 반복되는 일을 스크립트로 만드는 Lambda가 기본으로 들어 있습니다. 팀이 필요한 일을 맡는 Planner는 나머지 봇들과 함께 설정에서 추가할 수 있습니다. 이름 하나와 무엇을 위한 봇인지 한 문장이면 내 봇도 만들 수 있고, 실제 결정이 필요할 때는 봇이 나에게 묻습니다.
...
| | 제공자 |
|---|---|
| **음성** | OpenAI GPT-Live 1 (기본 백엔드 GPT-5.6 Luna) |
