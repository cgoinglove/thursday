...
2. **Serve a copy on its own empty data folder** for anything that needs clicking, fake states or data: `bash .agents/skills/verify/serve.sh start` (prints the URL), and `bash .agents/skills/verify/serve.sh stop` when done — always, even after a failure. It picks a free port and never stops a server it did not start. When another `next dev` already holds this folder (Next allows one), it builds and serves the build instead; that takes a minute.

Then drive it headless with `node .agents/skills/verify/look.cjs <url> <out.png> [width] [height]` for one screen, or copy `look.cjs` next to it and script the steps (fill, press, wait for an element). Read every screenshot before saying it works, and say what page errors it printed.

- Never headed: a window on the user's screen is theirs, not a check's — it takes focus, and on macOS a Chrome-family window can take the links they open.
