...
`parentId` identifies the conversation to which a continuation returns. It does not identify the participant. A message is a call, and the callee's final text is its return. If A waits for B and B needs something from A first, B ends its turn with that question; A reads it as B's return and answers with a new message, which resumes B on the same transcript, and A keeps its original responsibility to report to Thursday. B cannot message A while it is answering A. Only exchanged messages enter the other participant's context. The human can inspect all recorded activity; workspace files remain shared resources.

## Messages and turns

Bots use `send_message(to, text, kind?, options?)`. A send commits before returning a receipt. The sender can continue independent work or end its turn. A natural final message returns to the current correspondent; a silent ending is valid. There is no completion tool or required closing phrase.

A send to another bot opens an exchange, unless that bot is already on one from the sender: then the words join it and are read before its next step, so one job returns once. A send to the bot being answered is refused, since the turn's final text is what reaches it; sent as a message it would open an exchange the other way, and each side's ending would wake the other in turn. For Thursday, `kind: "message"` (the default) sends an update without opening a question or delaying completion. Use `kind: "question"` for user input; it carries an addressable question ID and optional answer choices. Other participants continue while a question is open, but the question ends the asker's turn: until it is answered, that bot runs nothing, messages to it wait in its inbox, and words from the user side addressed to it answer the question. With several questions open and none named, an answer is refused with the list. The screen and the call's `thread` tool can direct an answer or a new message to a particular participant. An answer is words like any other: a question that offers `Continue` among its choices is answered by it, and only a stop the app made is resumed by that word.
...
The coordinator's normal final message completes the thread only when all work has settled. If activity settles without a coordinator report, the scheduler permits one wrap-up turn. An empty final turn leaves an idle, resumable room; it does not cause an automatic retry loop. A participant whose earlier return is still unread and that then ends silently sends nothing more; that return is the answer.
...
A thread is created together with its coordinator's first continuation, so every thread is a room from its first row.

Thursday relays are persisted independently of whether a thread has been opened on screen. Unaccepted reports remain in the inbox beyond the usual finished-thread limit. Opening a thread, or Thursday marking it seen, settles its relays; so does a resume for the stop it picks up from, and new words for how it last stopped or ended. A call accepts a relay once Live acknowledges the update; this is not proof that the user heard the audio, and an update Live never acknowledged is relayed again on a later call. A question or ending she voiced is not presented again on a later call while the page stays open; it stays on screen until handled.
