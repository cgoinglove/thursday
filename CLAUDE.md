...
- **Anything private is named `*.local.*`** — a scratch note, a task list, a plan, a local
  ...
  accident. `CLAUDE.local.md` is the working example: how you like to work with an agent — when to
  ask, how to commit — goes there, and Claude Code loads it beside this file. This file holds only
...
scripts/dev.mts           `pnpm dev`: `next dev` on a port nothing holds on any address, handed over as `PORT`.
