...
4. Look at it once when a page carries a picture you did not draw — a photo you
   ...
   That is every page as one picture for a single `look_at`. It opens a browser
   of its own, which is this job's alone — no other bot's window is touched. Fix
   what it shows, then stop.
