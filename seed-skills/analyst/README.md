...
| Skill | What it adds |
|---|---|
| `data-report` | Seven report forms, each with its structure and a filled example; `fetch.mjs`, which pulls a published series (FRED, World Bank, Yahoo Finance, ECB rates, SEC filings, Wikipedia pageviews) into a CSV that names its source; `chart.mjs`, which draws a CSV into a quick page as a cited figure |
...
`chart.mjs` fills a page made by the shipped `interactive-page` skill's quick path and uses its
colour tokens; it draws inline SVG, so the page still opens with no network. The scripts need Node
and no packages. Every source they call answers without a key; `references/sources.md` names the
ones that need one.
