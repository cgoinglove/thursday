...
1. Check it is there: `tool_search` with `server: "studio"` and `tools: ["transcribe"]`. A
   name that does not come back means no transcription model is picked; say so in your
   answer (Settings › Models) and give what the title, description and chapters tell.
2. `node $S/audio.mjs cut <url|file> --out <scratch>/audio` downloads the audio (any page
   yt-dlp knows, a direct mp3 url, or a file) and cuts it into ten-minute pieces. It
   installs yt-dlp and ffmpeg into `projects/` the first time, once.
...
4. `node $S/audio.mjs join <scratch>/audio/<name>.pieces.json <file> <file>…` with those
   ...
   A time marked `~` is estimated from where the words fall in its piece.
