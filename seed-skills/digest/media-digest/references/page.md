...
- **Audio when asked** ("read it to me", "something to listen to"): write the script for the
  ...
  Put the file in the page's `audio` too. No speech model (`tool_search` does not return
  `generate_speech`): say so and give the text.
...
- `points` in the order they come in the source, each with its `at` from the transcript
  line it rests on. Five to seven, unless asked for a number. The page draws them on a line
  across the video's length.
