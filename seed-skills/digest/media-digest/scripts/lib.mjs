...
/** "1:02:05", "12:30", "95", 95 → seconds; null when it is none of these. */
...
export function readingLine(file, index) {
  ...
  return `${chars} chars in ${index.length} parts of ≤${PART_CHARS.toLocaleString("en")} chars: node ${shown(join(dirname(new URL(import.meta.url).pathname), "part.mjs"))} ${shown(file)} <1-${index.length}>`;
}
...
/**
 * A GET with a browser's user agent. `SOCS` is the answer to the EU consent page, which
 * otherwise stands in for every YouTube page from there.
 */
export async function get(url, { json = false, headers = {} } = {}) {
