...
//       file, and writes <dir>/<name>-001.mp3… (mono, small) and <dir>/<name>.pieces.json.
...
async function cut(input, flags) {
  ...
  return [
    ...
    `Transcribe each piece, then: node ${shown(join(dirname(new URL(import.meta.url).pathname), "audio.mjs"))} join ${shown(manifest)} <the transcript files, in this order>`,
  ].join("\n");
}
